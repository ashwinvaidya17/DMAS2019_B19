...
def rpsls_run(total_no_of_runs,p1order,p2order,game_no):
        ...
        while turn < 2:
            ...
            if turn == 1:
                ...
                if p2_order=="random":
                    ...
                    if count > res_limit:
                        p2=tom.agents_functionality("p2",p2_order,p2_ch,p1_ch)
                        print("player two ("+str(p2_order)+" order) choice at round "+str(count)+":",rpsls[p2])
                        p2_ch.append(p2)
                    else:
                        p2=random.randint(1,5)
    ...
    game_summary,score_p1,score_p2,match_draw,p1_score,p2_score,draw=a.compute_res(game_no)
    with open(file_to_save_result,'a') as result:
        result.write("The choices of player 1 in game "+str(game_no)+":")
        result.write(str(p1_ch))
        result.write("\n")
        result.write("The choices of player 2 in game "+str(game_no)+":")
        result.write(str(p2_ch))
        result.write("\n")
        result.write("The result of the game "+str(game_no)+":")
    ...
    return score_p1,score_p2,match_draw,p1_score,p2_score,draw
